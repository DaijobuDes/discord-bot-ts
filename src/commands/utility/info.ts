  ...
  .addSubcommand(subcommand =>
    subcommand
      ...
      .addUserOption((option: SlashCommandUserOption) =>
        option.setName('target').setDescription('The user to be fetched')
      )
      .addBooleanOption((option: SlashCommandBooleanOption) =>
        option
          .setName('ephemeral')
          .setDescription(
            'Set to false if you want to let everyone see the message.'
          )
      )
  )
  .addSubcommand(subcommand =>
...
export async function execute(
  ...
  if (interaction.options.getSubcommand() === 'user') {
    ...
    if (!guild) {
      await interaction.reply({
        content: 'You are not on a guild!',
        flags: MessageFlags.Ephemeral,
      });
      return;
    }
    ...
    const createdAtTimestampSnowflake = Math.round(
    ...
    embed.setThumbnail(
      `https://cdn.discordapp.com/avatars/${userId}/${avatarId}?size=4096`
    );
